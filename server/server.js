...
import { createRequire } from "module"; // this is the fix for JSON import
...
const serviceAccountKey = require("./blogging-website-b3f42-firebase-adminsdk-fbsvc-a6756cb1c9.json"); // ✅ fixed import
  ...
  getAuth()
    ...
    .then(async (decodedUser) => {
      ...
      if (user) {
        if (!user.google_auth)
          return res.status(403).json({
            error:
              "This email was not signed up using Google. Please use password login.",
          });
      } else {
        let username = await generateUsername(email);
        user = new User({
    ...
    .catch((err) => {
      return res.status(500).json({
        error:
          "Failed to authenticate with Google. Try a different Google account.",
      });
    });
});
...
server.post("/search-blogs", (req, res) => {
  ...
  if (tag) {
    findQuery = { tags: tag, draft: false, blog_id: { $ne: eliminate_blog } }; //mongo func
  } else if (query) {
...
server.post("/update-profile", verifyJWT, (req, res) => {
  ...
  if (username.length < 3) {
    return res
      .status(403)
      .json({ error: "Username should be atleast 3 characters" });
  }
...
server.post("/create-blog", verifyJWT, (req, res) => {
  ...
  if (!draft) {
    ...
    if (!tags.length || tags.length > 10) {
      return res
        .status(403)
        .json({ error: "Provide tags in order to publish the blog(max 10)" });
    }
  }
...
server.post("/add-comment", verifyJWT, (req, res) => {
  ...
  if (!comment.length) {
    return res.status(400).json("Write something to leave a comment...");
  }
